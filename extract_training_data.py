...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(1, 4):
            if len(stack) < i:
                ...
                l.append(self.word_vocab['<ROOT>'])
            elif pos[stack[-i]] == 'CD':
                l.append(self.word_vocab['<CD>'])
            elif pos[stack[-i]].find('NNP') == 0:
                l.append(self.word_vocab['<NNP>'])
            elif words[stack[-i]].lower() not in self.word_vocab:
                l.append(self.word_vocab['<UNK>'])
            else:
                l.append(self.word_vocab[words[stack[-i]].lower()])
        ...
        for i in range(1, 4):
            if len(buffer) < i:
                ...
                l.append(self.word_vocab['<CD>'])
            elif pos[buffer[-i]].find('NNP') == 0:
                l.append(self.word_vocab['<NNP>'])
            elif words[buffer[-i]].lower() not in self.word_vocab:
                l.append(self.word_vocab['<UNK>'])
            else:
                l.append(self.word_vocab[words[buffer[-i]].lower()])
    ...
    def get_output_representation(self, output_pair):  
        ...
        num = keras.utils.to_categorical([self.output_labels[output_pair]], num_classes=91)
        return num
